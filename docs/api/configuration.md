...
文档路径上方最多允许 5 个目录层级，例如 `a/b/c/d/e/doc.md` 可以使用；超过第 5 个目录层级的 Markdown 文件会被拒绝。服务和静态构建最多加载 1000 篇 Markdown 文档，隐藏文档和超大 Markdown 文件也会计入扫描限制。超过任一限制时，服务接口会返回明确错误，静态构建会终止，不会生成不完整的导航树。
