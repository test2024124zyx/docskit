...
- 文档路径最多包含 5 个目录层级，站点最多加载 1000 篇 Markdown 文档；规划或交付文档集合时必须检查这两项限制。隐藏文档和超大 Markdown 文件也计入文档数量上限。
...
- 文档路径上方的目录层级不超过 5 级，文档集合不超过 1000 篇；不要把这两个运行时固定限制写成 `docs.config.json` 字段。
